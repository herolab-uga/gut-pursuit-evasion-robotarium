...
# get the surround point
def get_surroundpoint(x, i, numPursuer):
    radius = 0.2
    evaderPose = x[:, 0]
    theta = (2 * np.pi) / numPursuer * i

    surroundPointX = evaderPose[0] + radius * np.cos(theta)
    surroundPointY = evaderPose[1] + radius * np.sin(theta)

    return np.array([[surroundPointX], [surroundPointY]])

def get_shrinkpoint(x, i, numPursuer):
    radius = 0.3
    evaderPose = x[:, 0]
    theta = np.pi / 4

    shrinkPointX = evaderPose[0]  + radius * np.sin(theta) * (np.floor(i/2) * np.power(-1, i) - 1)
    shrinkPointY = evaderPose[1] + radius * np.cos(theta) * (np.floor(i/2) * np.power(-1, i) + 1)

    return np.array([[shrinkPointX], [shrinkPointY]])
...
def gut_pursuit_game():
    ...
    initial_conditions = np.array([[-0.5, 0, -1.4, 1.4, -1.4, 1.4],[0.121, 0.8, -0.8, -0.8, 0, 0],[0, 0, 0, 0, 0, 0]])
    ...
    pursuer4_label = r.axes.text(x[0,4],x[1,4]+0.2,"pursuer 4",fontsize=font_size1, color='b',fontweight='bold',horizontalalignment='center',verticalalignment='center',zorder=0)
    pursuer4_energy_label = r.axes.text(x[0,4],x[1,4]+0.2,"NRG: ",fontsize=font_size2, color='c',fontweight='bold',horizontalalignment='center',verticalalignment='center',zorder=0)
    pursuer4_hp_label = r.axes.text(x[0,2],x[1,2]+0.15,"Dist: ",fontsize=font_size2, color='m',fontweight='bold',horizontalalignment='center',verticalalignment='center',zorder=0)

    pursuer5_label = r.axes.text(x[0,5],x[1,5]+0.25,"pursuer 5",fontsize=font_size1, color='b',fontweight='bold',horizontalalignment='center',verticalalignment='center',zorder=0)
    pursuer5_energy_label = r.axes.text(x[0,5],x[1,5]+0.2,"NRG: ",fontsize=font_size2, color='c',fontweight='bold',horizontalalignment='center',verticalalignment='center',zorder=0)
    pursuer5_hp_label = r.axes.text(x[0,3],x[1,3]+0.15,"Dist: ",fontsize=font_size2, color='m',fontweight='bold',horizontalalignment='center',verticalalignment='center',zorder=0)
    ...
    for k in range(iterations):
        ...
        pursuer4_label.set_position([x_si[0,4],x_si[1,4]+0.25])
        pursuer4_label.set_fontsize(determine_font_size(r,font_size_m1))
        pursuer4_energy_label.set_position([x_si[0,4],x_si[1,4]+0.2])
        pursuer4_energy_label.set_fontsize(determine_font_size(r,font_size_m2))
        pursuer4_energy_label.set_text("NRG: " + str(round(agent_energy_level[4], 2)))
        pursuer4_hp_label.set_position([x_si[0,4],x_si[1,4]+0.15])
        pursuer4_hp_label.set_fontsize(determine_font_size(r,font_size_m2))
        pursuer4_hp_label.set_text("Dist: " + str(round(pursuer_evader_distance[4], 2)))

        pursuer5_label.set_position([x_si[0,5],x_si[1,5]+0.25])
        pursuer5_label.set_fontsize(determine_font_size(r,font_size_m1))
        pursuer5_energy_label.set_position([x_si[0,5],x_si[1,5]+0.2])
        pursuer5_energy_label.set_fontsize(determine_font_size(r,font_size_m2))
        pursuer5_energy_label.set_text("NRG: " + str(round(agent_energy_level[5], 2)))
        pursuer5_hp_label.set_position([x_si[0,5],x_si[1,5]+0.15])
        pursuer5_hp_label.set_fontsize(determine_font_size(r,font_size_m2))
        pursuer5_hp_label.set_text("Dist: " + str(round(pursuer_evader_distance[5], 2)))
        ...
        for i in range(N):
            ...
            if i >= 1:
                oldpoint, catchSurroundpoint = get_catchpoint(np.append(get_surroundpoint(x, i, N-1), x[2,0]), x[:, i], oldpoint)
                oldpoint, catchShrinkpoint = get_catchpoint(np.append(get_shrinkpoint(x, i, N-1), x[2,0]), x[:, i], oldpoint)

                if (np.array(pursuer_evader_distance) > 0).all():
                    pursuer_strategy, _ = gut_pursuit.GUT_DecisionMaking(N-1, 1, agent_energy_level, pursuer_evader_distance)
                else:
                    pursuer_strategy = ['circle', 'cb']

                # pursuer_strategy = ['semicircle', 'cb']
                # pursuer_strategy = ['circle', 'cb']

                if pursuer_strategy[0] == 'semicircle' and pursuer_strategy[1] == 'pp':
                    si_velocities[:, i] = np.sum(get_shrinkpoint(x, i, N-1)[:, 0, None] - x_si[:, i, None], 1)
                elif pursuer_strategy[0] == 'semicircle' and pursuer_strategy[1] == 'cb':
                    si_velocities[:, i] = np.sum(catchShrinkpoint[:, 0, None] - x_si[:, i, None], 1)
                elif pursuer_strategy[0] == 'circle' and pursuer_strategy[1] == 'pp':
                    si_velocities[:, i] = np.sum(get_surroundpoint(x, i, N-1)[:, 0, None] - x_si[:, i, None], 1)
                elif pursuer_strategy[0] == 'circle' and pursuer_strategy[1] == 'cb':
                    si_velocities[:, i] = np.sum(catchSurroundpoint[:, 0, None] - x_si[:, i, None], 1)
